...
int Index,top;
bool Instack[MAXN];
bool cut[MAXN];//某点是否为割顶
int add_block[MAXN];//删除一个点后增加的连通块（点-双连通分量？）
int bridge;
void addedge(int u,int v)//单向加边
{
...
void Tarjan(int u,int pre)
{
    ...
    for(int i = head[u];i != -1;i = edge[i].next)
    {
        ...
        if( !DFN[v] )
        {
            ...
            if(Low[v] > DFN[u])
            {
                bridge++;  edge[i].cut = true; edge[i^1].cut = true;
            }
            //割点
