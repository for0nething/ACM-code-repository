...
//也就是说dis得单独计算//dfs2一次比两次少一半多的常数=_=
...
struct node {
    ...
    node() {}
    node(int _top,int _sub,int _len,int _next):top(_top),sub(_sub),len(_len),next(_next) {};
} nodes[maxn*20];
...
void dfs3(int x) {
    ...
    for (int i=head[x]; ~i; i=edge[i].next) {
        ...
        maxdep=0; dfs2(v,0,x,root,1); //判重是x,init_dep=1
