...
int i,j,k;
char c;
char buffer[12000000],*buf=buffer,out[7000000],*ed=out;
void read(int &x) {
	for (x=0; *buf<48; ++buf);
...
int main() {
	...
	FOR(i,1,Q) {
		read(q[i].op);
		if (q[i].op==1) {
			read(q[i].i); c=*++buf; buf++; q[i].c=c-'a';
			trie.Add(q[i].i,q[i].c);
		} else if (q[i].op==2) {
			read(q[i].i); read(q[i].j); c=*++buf; buf++; q[i].c=c-'a';
		} else read(q[i].i),read(q[i].j);
	}
	trie.buildfa();
	trie.buildSAM();
	trie.linkfail();
	...
	getans(0);
	FOR(i,1,Q) if (q[i].op==4) print(Ans[i]);
	fwrite(out,1,ed-out,stdout);
	return 0;
}
/*
*/
